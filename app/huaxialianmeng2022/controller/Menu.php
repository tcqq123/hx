...
use app\huaxialianmeng2022\model\AdminMenu as menuModel;
class Menu extends Permissions
{
		...
		if($id > 0) {
    		...
    		if($this->request->isPost()) {
	            ...
	            if(false == $model->allowField(true)->save($post,['id'=>$id])) {
	            	...
	            	return $this->success('修改菜单信息成功','huaxialianmeng2022/menu/index');
	            }
    		} else {
    			//非提交操作
    			$menu = $model->where('id',$id)->find();
    		...
    		if($this->request->isPost()) {
	            ...
	            if(false == $model->allowField(true)->save($post)) {
                    ...
                    addlog($model->id);//写入日志
	            	return $this->success('添加菜单成功','huaxialianmeng2022/menu/index');
	            }
    		} else {
    			//非提交操作
    	...
    	if($this->request->isAjax()) {
            ...
            if(Db::name('admin_menu')->where('pid',$id)->select() == null) {
                if(false == Db::name('admin_menu')->where('id',$id)->delete()) {
                    ...
                    addlog($id);//写入日志
                    return $this->success('删除成功','huaxialianmeng2022/menu/index');
                }
            } else {
    ...
    public function orders()
    {
        if($this->request->isPost()) {
            ...
            addlog();//写入日志
            return $this->success('成功更新'.$i.'个数据','huaxialianmeng2022/menu/index');
        }
    }
}
