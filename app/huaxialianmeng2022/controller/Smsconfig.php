...
class Smsconfig extends Permissions
{
    ...
    public function publish()
    {
    	if($this->request->isPost()) {
            ...
            if(false == Db::name('smsconfig')->where('sms','sms')->update($post)) {
                ...
                addlog();//写入日志
            	return $this->success('提交成功','huaxialianmeng2022/smsconfig/index');
            }
    	} else {
    ...
    public function smstos()
    {
        $id = input('post.');
        $datas = Db::table('app_mobile')->where('userid',$id['id'])->select();
       
        return json(['code' => 1, 'msg' => $datas]);
