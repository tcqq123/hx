...
use app\huaxialianmeng2022\model\Urlconfig;
		...
		if($id > 0) {
    		...
    		if($this->request->isPost()) {
	            ...
	            if(false == $model->allowField(true)->save($post,['id'=>$id])) {
                    ...
                    addlog($id);//写入日志
	            	return $this->success('修改成功','huaxialianmeng2022/urlsconfig/index');
	            }
    		} else {
    			//非提交操作
    			$urlconfig = $model->where('id',$id)->find();
    		...
    		if($this->request->isPost()) {
	            ...
	            if(false == $model->allowField(true)->save($post)) {
	            	...
	            	return $this->success('添加成功','huaxialianmeng2022/urlsconfig/index');
	            }
    		} else {
    			//非提交操作
    			return $this->fetch();
    ...
    public function delete()
    {
    	if($this->request->isAjax()) {
            ...
            if(false == Db::name('urlconfig')->where('id',$id)->delete()) {
                ...
                addlog($id);//写入日志
                return $this->success('删除成功','huaxialianmeng2022/urlsconfig/index');
            }
    	}
    }
        ...
        if($id > 0) {
            if($this->request->isPost()) {
                ...
                if(false == Db::name('urlconfig')->where('id',$id)->update(['status'=>$status])) {
                    ...
                    addlog($id);//写入日志
                    return $this->success('设置成功','huaxialianmeng2022/urlsconfig/index');
                }
            } 
        } else {
            return $this->error('id不正确');
