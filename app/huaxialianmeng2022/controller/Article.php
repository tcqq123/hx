...
use app\huaxialianmeng2022\model\Article as articleModel;
use app\huaxialianmeng2022\model\ArticleCate as cateModel;
		...
		if($id > 0) {
    		...
    		if($this->request->isPost()) {
	            ...
	            if(false == $model->allowField(true)->save($post,['id'=>$id])) {
	            	...
	            	return $this->success('修改成功','huaxialianmeng2022/article/index');
	            }
    		} else {
    			//非提交操作
    			$article = $model->where('id',$id)->find();
    		...
    		if($this->request->isPost()) {
	            ...
	            if(false == $model->allowField(true)->save($post)) {
                    ...
                    addlog($model->id);//写入日志
	            	return $this->success('添加成功','huaxialianmeng2022/article/index');
	            }
    		} else {
    			//非提交操作
    ...
    public function delete()
    {
    	if($this->request->isAjax()) {
            ...
            if(false == Db::name('article')->where('id',$id)->delete()) {
                ...
                addlog($id);//写入日志
                return $this->success('删除成功','huaxialianmeng2022/article/index');
            }
    	}
    }
    ...
    public function is_top()
    {
        if($this->request->isPost()){
            ...
            if(false == Db::name('article')->where('id',$post['id'])->update(['is_top'=>$post['is_top']])) {
                ...
                addlog($post['id']);//写入日志
                return $this->success('设置成功','huaxialianmeng2022/article/index');
            }
        }
    }
