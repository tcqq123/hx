        ...
        if($id > 0) {
            ...
            if($this->request->isPost()) {
                ...
                if(false == $model->allowField(true)->save($post,['id'=>$id])) {
                    ...
                    return $this->success('修改个人信息成功','huaxialianmeng2022/admin/personal');
                }
            } else {
                //非提交操作
    ...
    public function publish()
    {
    	...
    	if($id > 0) {
    		...
    		if($this->request->isPost()) {
	            ...
	            if(empty($post['type'])) {
					return $this->error('提交失败：请选择分类');
				}
	            if(false == $model->allowField(true)->save($post,['id'=>$id])) {
	            	return $this->error('修改失败');
	            } else {
                    addlog($model->id);//写入日志
	            	return $this->success('修改管理员信息成功','huaxialianmeng2022/admin/index');
	            }
    		} else {
    			//非提交操作
    			$info['admin'] = $model->where('id',$id)->find();
            ...
            if ($count > config('agency_number')) {
                return $this->error('添加代理超过'.config('agency_number').'人，无法继续添加');
            }
    		//是新增操作
    		if($this->request->isPost()) {
    			//是提交操作
    			$post = $this->request->post();
    			$overdue_time = $post['overdue_time'];
    			$post['overdue_time'] = strtotime($overdue_time);
    			//验证  唯一规则： 表名，字段名，排除主键值，主键名
	            $validate = new \think\Validate([
	            ...
	            if(false == $model->allowField(true)->save($post)) {
                    ...
                    while ($bool) {
                        ...
                        $bool = Db::name('appconfig')->where('yaoqingma',$code)->find();
                    }
                    if (!$bool) {
                        $app = Db::name('appconfig')->insert([
                            'app' => 'agency',
                            'is_login' => 1,
                            'is_reg' => 0,
                            'yaoqingma' => $code,
                        ...
                        if (!$app) {
                            return $this->error('添加管理员邀请码失败');
                        }
                    }

	            	return $this->success('添加管理员成功','huaxialianmeng2022/admin/index');
	            }
    		} else {
    			//非提交操作
    			$info['admin_cate'] = Db::name('admin_cate')->select();
    		...
    		if($id == Session::get('admin')) {
    			...
    			if(password($post['password_old']) == $admin['password']) {
    				if(false == Db::name('admin')->where('id',$id)->update(['password'=>password($post['password'])])) {
                        ...
                        addlog();//写入日志
    					return $this->success('修改成功','huaxialianmeng2022/main/index');
    				}
    			} else {
    ...
    public function delete()
    {
    	if($this->request->isAjax()) {
    		...
    		if(false == Db::name('admin')->where('id',$id)->delete()) {
                ...
                addlog($id);//写入日志
    			return $this->success('删除成功','huaxialianmeng2022/admin/index');
    		}
    	}
    }
        ...
        if($id > 0) {
            ...
            if($this->request->isPost()) {
                ...
                if(false == $model->allowField(true)->save($post,['id'=>$id])) {
                    ...
                    addlog($model->id);//写入日志
                    return $this->success('修改角色信息成功','huaxialianmeng2022/admin/adminCate');
                }
            } else {
                //非提交操作
            ...
            if($this->request->isPost()) {
                ...
                if(false == $model->allowField(true)->save($post)) {
                    ...
                    addlog($model->id);//写入日志
                    return $this->success('添加角色成功','huaxialianmeng2022/admin/adminCate');
                }
            } else {
                //非提交操作
                $menus = Db::name('admin_menu')->select();
        ...
        if($this->request->isAjax()) {
            ...
            if($id > 0) {
                ...
                if(false == Db::name('admin_cate')->where('id',$id)->delete()) {
                    ...
                    addlog($id);//写入日志
                    return $this->success('删除成功','huaxialianmeng2022/admin/adminCate');
                }
            } else {
                return $this->error('id不正确');
            }
