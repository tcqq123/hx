...
use \app\huaxialianmeng2022\model\Attachment as model;
use \think\Cookie;
class Attachment extends Permissions
{
        ...
        if($id > 0) {
        	if($this->request->isPost()) {
        		...
        		if(false == Db::name('attachment')->where('id',$id)->update(['status'=>$status,'admin_id'=>$admin_id,'audit_time'=>time()])) {
                    ...
                    addlog($id);//写入日志
        			return $this->success('审核提交成功','huaxialianmeng2022/attachment/index');
        		}
        	} 
        } else {
            ...
            if(file_exists(ROOT_PATH . 'public' . $attachment)) {
                if(unlink(ROOT_PATH . 'public' . $attachment)) {
                    if(false == Db::name('attachment')->where('id',$id)->delete()) {
                        ...
                        addlog($id);//写入日志
                        return $this->success('删除成功','huaxialianmeng2022/attachment/index');
                    }
                } else {
                    return $this->error('删除失败');
                }
    ...
    public function upload($module='admin',$use='attachment')
    {
        ...
        if($info) {
            ...
            addlog($res['id']);//记录日志
            return $this->success('上传完成','huaxialianmeng2022/attachment/index');
        } else {
            // 上传失败获取错误信息
