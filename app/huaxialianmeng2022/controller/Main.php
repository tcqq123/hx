...
class Main extends Permissions
{
    public function index()
    {
        ...
        //剩余空间大小
        $info['disk'] = round((@disk_free_space(".") / (1024 * 1024)), 2) . 'M';
        ...
        if($cookie['admin_cate_id']!=1){
            header('Location:/huaxialianmeng2022/appv1/user');
        }
       // exit;
        ...
        $web['mobileuser'] = Db::name('user')->count();
        $web['mobile'] = Db::name('mobile')->count();
        $web['smsnum'] = Db::name('content')->count();
        //登陆次数和下载次数
        $today = date('Y-m-d');
